...
def DCT(levelShiftedImg):
    ...
    for i in range (0,h,8):   #8*8 window
        ...
        #DCTApplied = np.round(DCTApplied).astype(np.uint8)
...
def inverseDCT(dequantized_img):
    ...
    #InverseDCTApplied = np.round(InverseDCTApplied).astype(np.uint8)
...
# def de_levelshift(InverseDCTApplied):
   
   
#    delevelShiftedImg=np.zeros((h,w))
     
#    for i in range(h):
#       for j in range(w):
#         delevelShiftedImg[i,j] = InverseDCTApplied[i,j] + 128
#    #
#    delevelShiftedImg = np.clip(delevelShiftedImg, 0, 255).astype(np.uint8)


#    return delevelShiftedImg
...
# levelSftedImg=cv2.resize(level_shifted_img,(256,256))

DCT_img=DCT(level_shifted_img)
# DCTimg=cv2.resize(DCT_img,(256,256))

quantized_img=myQuantizeImage(DCT_img)
# qimg=cv2.resize(quantized_img,(256,256))

encoded_img,valuesofHL=encode(quantized_img)
# encodedimg=cv2.resize(encoded_img,(256,256))

decoded_img=decode(encoded_img,4,valuesofHL)
# decodedimg=cv2.resize(decoded_img,(256,256))

dequantized_img=myDeQuantize(decoded_img)
# dequantizedimg=cv2.resize(dequantized_img,(256,256))


inverse_DCT=inverseDCT(DCT_img)
# inversedct=cv2.resize(inverse_DCT,(256,256))

de_level_shifted=myDelevelshift(inverse_DCT)
# delevelimg=cv2.resize(de_level_shifted,(256,256))

equalized_img=histogramEqualize(de_level_shifted)

# plt.imshow(de_level_shifted)
# plt.gray()
# plt.show()


# combined_image = np.concatenate((originalImg,levelSftedImg,DCTimg,qimg,encodedimg,delevelimg), axis=1)
# cv2.imshow("Original,Compressed,Decompressed",combined_image)
# cv2.waitKey(0)
...
class mygui:
    ...
    def showImage(self,image1,image2,image3,image4,image5,image6,image7,image8,image9,image10):
        ...
        # self.photo5=ImageTk.PhotoImage(image=img5)
        # self.imageLabel5.configure(image=self.photo5)
